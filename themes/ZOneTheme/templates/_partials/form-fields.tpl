...
{else}
  ...
  <div class="form-group row {if !empty($field.errors)}has-error{/if}">
    <label class="col-12 col-md-3 form-control-label{if $field.required} required{/if}{if $field.type == 'checkbox'} hidden-sm-down{/if}">
      {if $field.type !== 'checkbox'}
        {$field.label}
      {/if}
    </label>
    <div class="col-12 {if in_array($field.type, ['radio-buttons', 'checkbox'])}col-md-9{else}col-md-6{/if} {if ($field.type === 'radio-buttons')}form-control-valign{/if}">
    ...
    {if !in_array($field.type, ['radio-buttons', 'checkbox'])}
      {block name='form_field_comment'}
        <div class="col-12 col-md-3 form-control-comment">
          {if (!$field.required)}
