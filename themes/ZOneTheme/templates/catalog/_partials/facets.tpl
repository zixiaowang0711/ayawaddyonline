...
<div id="search_filters">
  {block name='facets_title'}
    <h4 class="column-title hidden-sm-down">{l s='Filter By' d='Shop.Theme.Actions'}</h4>
  {/block}
  ...
  {foreach from=$facets item="facet"}
    {if $facet.displayed}
      <section class="facet">
        <h5 class="facet-title hidden-sm-down"><i class="fa fa-angle-double-right" aria-hidden="true"></i> {$facet.label}</h5>
                      ...
                      {if $facet.multipleSelectionAllowed}
                        <span class="custom-checkbox">
                          ...
                          {if isset($filter.properties.color)}
                            <span class="color" style="background-color:{$filter.properties.color}"><span class="check-circle"></span></span>
                          {elseif isset($filter.properties.texture)}
                            <span class="color texture" style="background-image:url({$filter.properties.texture})"><span class="check-circle"></span></span>
                          {else}
                            <span {if !$js_enabled} class="ps-shown-by-js" {/if}><i class="material-icons check-icon">&#xE5CA;</i></span>
                          {/if}
                        </span>
                      {else}
