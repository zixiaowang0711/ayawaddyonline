...
class NetReviewsModel extends ObjectModel
{
    ...
    public function getProductReviews($id_product, $group_name = null, $id_shop = null, $count_reviews = false, $p = 1)
    {
        ...
        if ($count_reviews) {
            $sql = 'SELECT COUNT(ref_product) as nbreviews FROM '._DB_PREFIX_
                .'av_products_reviews WHERE ref_product = '.(int)$id_product;
        } else {
    ...
    public function export($header_colums, $id_shop = null)
    {
        ...
        $qry_sql = '    SELECT o.module, lg.iso_code, o.id_order, o.total_paid, o.id_customer, o.date_add,'.
            ' c.firstname, c.lastname, c.email '.$select_id_shop.$select_id_state.'
            ...
            if ($csv = @fopen($file_path, 'w')) {
                ...
                foreach ($all_orders as $order) {
                    ...
                    if ($avis_produit == 1 && $count_products > 0) {
                        ...
                        $num_state = $order['ORDER_STATE'];
                        $line[] = $num_state; //Order state ID
                        $line[] = $order_statut_list[$num_state]['name']; //Order state
                        $line[] = $order['ISO_LANG']; //Order lang
                        if (! empty($id_shop)) {
                            $line[] = $id_shop;
                        }
    ...
    public static function getUrlsProduct($product_id)
    {
        ...
        if ($product_exist) {
            ...
            if (version_compare(_PS_VERSION_, '1.7', '>')) {
                $image_path = $link->getImageLink($o_product->link_rewrite, $id_cover_image['id_image'],'large_default');
            }else{
                $image_path = $link->getImageLink($o_product->link_rewrite, $id_cover_image['id_image'],'thickbox_default');
            }
    ...
    public static function acDecodeBase64($s_data)
    {
     ...
     public static function toUTF8($text){
      ...
      public static function l($string) { //override category stars translations 
           return Translate::getModuleTranslation('netreviews', $string, 'avisverifies-categorystars');
      }
