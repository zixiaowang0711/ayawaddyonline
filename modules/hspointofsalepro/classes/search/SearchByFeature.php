 ...
 *
 */
class SearchByFeature extends SearchAbstract
{
    ...
    public function search()
    {
        ...
        $product_query->innerJoin('feature_product', 'fp', 'fp.`id_product` = p.`id_product`');
        $product_query->innerJoin('feature_shop', 'fs', 'fs.`id_feature` = fp.`id_feature` AND fs.`id_shop` = ' . (int) $this->shop->id);
        $product_query->innerJoin('feature_value', 'fv', 'fv.`id_feature` = fp.`id_feature` AND fv.`id_feature_value` = fp.`id_feature_value`');
        $product_query->innerJoin('feature_value_lang', 'fvl', 'fvl.`id_feature_value` = fv.`id_feature_value` AND fvl.`id_lang` = ' . (int) $this->id_lang);
        $product_query->join(Product::sqlStock('p', 0, true, $this->shop));
        $product_query->where('fvl.`value` LIKE \'%'.pSQL($this->keyword).'%\'');
