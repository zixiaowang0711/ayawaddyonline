...
class SearchByAttribute extends SearchAbstract
{
    ...
    public function search()
    {
        ...
        $product_query->leftJoin('product_lang', 'pl', 'p.`id_product` = pl.`id_product` AND pl.`id_lang` = ' . (int) $this->id_lang. ' AND pl.`id_shop` = '. (int) $this->shop->id);
        $product_query->leftJoin('product_attribute_combination', 'pac', 'pac.`id_product_attribute` = pa.`id_product_attribute`');
        $product_query->leftJoin('attribute', 'a', 'a.`id_attribute` = pac.`id_attribute`');
        $product_query->leftJoin('attribute_shop', 'ats', 'ats.`id_attribute` = a.`id_attribute` AND ats.`id_shop` = '.(int) $this->shop->id);
        $product_query->leftJoin('attribute_lang', 'al', 'al.`id_attribute` = a.`id_attribute` AND al.`id_lang` = ' . (int) $this->id_lang);
