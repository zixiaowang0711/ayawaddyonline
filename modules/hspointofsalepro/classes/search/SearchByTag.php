 ...
 *
 */
class SearchByTag extends SearchAbstract
{
    ...
    public function search()
    {
        ...
        $product_query->innerJoin('product_tag', 'pt', 'p.`id_product` = pt.`id_product`');
        $product_query->innerJoin('tag', 't', 't.`id_tag` = pt.`id_tag`');
        $product_query->join(Product::sqlStock('p', 0, true, $this->shop));
        $product_query->where('t.`name` LIKE \'%'.pSQL($this->keyword).'%\'');
