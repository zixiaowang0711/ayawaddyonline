 ...
 *
 */
class SearchByCategory extends SearchAbstract
{
    ...
    public function search()
    {
        ...
        $product_query->innerJoin('category_product', 'cp', 'cp.`id_product` = p.`id_product`');
        $product_query->innerJoin('category_lang', 'cl', 'cl.`id_category` = cp.`id_category` AND cl.`id_lang` = ' . (int) $this->id_lang);
        $product_query->join(Product::sqlStock('p', 0, true, $this->shop));
        $where_fields = array('name', 'description', 'link_rewrite', 'meta_title', 'meta_keywords', 'meta_description');
        $where = array();
        foreach ($where_fields as $where_field) {
            $where[] = 'cl.`'.$where_field.'` LIKE \'%'.pSQL($this->keyword).'%\'';
        }
        ...
        $product_query->orderBy('pl.`name` ASC');
